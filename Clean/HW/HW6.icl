...
*/
avg :: [Int] -> Real
avg list = toReal (sum list) / toReal (length list)

ExpandGrade :: [(String, [Int])] -> [(String, [Int])]
ExpandGrade listOfTuples
  = [ (name, [x]) \\ (name, grades) <- listOfTuples, x <- grades| x >= 85 && not (avg grades < 75.0) ]
...
indexLocation listOfLists
    | length listOfLists == 0 = []
    = [(list !! i !! i, i, i) \\ list <-listOfLists , i <- [0..(length listOfLists - 1)]]
