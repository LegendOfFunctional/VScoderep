...
isParasitic :: Int -> Int
isParasitic n = toInt (toString(n rem 10) +++ toString (n/10)) 

//Start = isParasitic 102564

parasitic :: Int Int -> Bool
parasitic n d 
| (n * d) == isParasitic n = True
= False
...
//Start = cntOcc 1 [121, 1, 2, 1, 2]

doubleOne :: [Int] -> [Int]
doubleOne [] = []
doubleOne [x : ls]
| (cntOcc 1 (toList x)) >= 2 = [x] ++ doubleOne ls
= doubleOne ls
...
//Start = multiple 2 // [2] - I think it is a wrong test case 
//Start = multiple 1 // []
...
repAux :: Int [Int] -> [Int]
repAux m ls = (take (((length ls)/2)) ls) ++ [m] ++ (drop (((length ls)/2) + 1) ls)

//Start = repAux [1, 2, 3, 4] 10 

repMid :: [[Int]] Int -> [[Int]]
repMid ls m = map (repAux m) ls
...
denominator :: Int -> [Int]
denominator x = filter (divisible x) [1..x]

isPrime :: Int -> Bool
isPrime x = denominator x == [1, x]
...
holdsAux [] = True
holdsAux [x : ls]
| (isEven (fst x)) && (isOdd (snd x)) = holdsAux ls
= False

holdsTrue :: [(Int, Int)] -> Bool
holdsTrue [] = False
holdsTrue ls = holdsAux ls
...
toDigits1 :: Int -> [Int]
toDigits1 0 = []
toDigits1 n = (toDigits1 (n/10)) ++ [(n rem 10)]

superAux :: Int -> Int 
superAux n
| n < 10 = n 
| otherwise = superAux (sum (toDigits1 n))

//Start = superAux 148148148

super_digit :: [Int] -> [Int]
super_digit [] = []
super_digit ls = map superAux ls
