...
countTuples :: [(Int, Int)] -> Int
countTuples ts = length (filter (\(x, y) -> x > y) ts)

tuples_with_property :: [[(Int,Int)]] -> [Int]
tuples_with_property ls = map (\l -> countTuples l) ls
...
createList :: Int -> [Int]
createList n = [n, n-1..1] ++ [1..n]

createLists :: Int -> [[Int]]
createLists n = [createList x \\ x <- [1..n]]

decreasing_list :: Int -> [[Int]]
decreasing_list n = (reverse (createLists n)) ++ (createLists n)
...
matchIndex ls f = all (\(idx, el) -> (f idx) == (f (length el))) [(x, y) \\ x <- [1..(length ls)] & y <- ls]
// matchIndex ls f = all (\(idx, el) -> (f idx) == (f (length el))) (zip2 (indexList l) l)
...
max_even_sum ls = getMax [sum x \\ x <- ls | (sum x) rem 2 == 0]
where
	getMax [] = -1
	getMax l = maxList l
...
isPrime :: Int -> Bool
isPrime n = all (\el -> n rem el <> 0) [2..n-1]

strToList :: String -> [Char]
strToList s = fromString s

isPalindrome :: Int -> Bool
isPalindrome n = (strToList (toString n)) == (reverse (strToList (toString n)))

extract :: [[Int]] -> [(Int, Int)]
extract ls = map (\l -> (hd (filter isPrime l), hd (filter isPalindrome l))) ls
...
primeFactors n
| isPrime n = [n]
= removeDup ([(getFirstPrime n)] ++ primeFactors (n / (getFirstPrime n)))
where
	getFirstPrime :: Int -> Int
	getFirstPrime n = filterFirst (\x -> isPrime x && n rem x == 0) [2..n]
...
areCoprime n1 n2 = not (any (\el -> (n1 rem el == 0) && (n2 rem el == 0)) [2..min n1 n2])
...
countPrimeUsage :: Int Int -> Int
countPrimeUsage n prime
| n rem prime == 0 = 1 + (countPrimeUsage (n / prime) prime)
= 0

prime_factors_mult :: Int -> [(Int, Int)]
prime_factors_mult n = [(x, countPrimeUsage n x) \\ x <- primeFactors n]
...
intersection l (start, end) = filter (\(elstart, elend) -> not ((elstart < start && elend < start) || (elstart > end && elend > end) )   ) l
...
j :: [(Int, Int, Int)] -> [Int]
j [] = []
j [(first, second, third):xs] = [first, second, third] ++ (j xs)

distribute3 :: [Int] -> [(Int, Int, Int)]
distribute3 [] = []
distribute3 l = [(l !! 0, l !! 1, l !! 2)] ++ (distribute3 (drop 3 l))

tuple_sort :: [(Int,Int,Int)] -> [(Int,Int,Int)]
tuple_sort ts = distribute3 (sort (j ts))
