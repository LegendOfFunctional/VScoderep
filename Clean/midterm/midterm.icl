...
*/
index :: Int [Int] -> Int
index n list = hd[i\\ x <-list & i <- [0..((length list)-1)]| x == n]
...
retrieve list n = removeDup[(x,y)\\ x <-sortedList, y<-sortedList |abs(x-y)==n ]
...
inc3 [x] = [x]
inc3 [x,y] = [x,y]
inc3 [x,y,z:xs] = [x, y,(z + x + y): inc3 xs]
...
mapFunList :: [Int] -> [[Int]]
mapFunList list = [[y x \\ y <- funList] \\ x <- list]
