...
truckParker n ls = countCons (removeMembers [1..n] ls)*/
...
tripCalc ls = map (\(a, op, b) = op (toInt a) (toInt b)) ls
...
toElement [] = 0
toElement [x] = x
toElement [x : ls] = ((x * 10) + toElement (ls)) / 10
...
//[1,2,3] 123 
//Start = toInt (foldl (+++) "" (map toString []))
...
isPolindrom :: [a] -> Bool | * a & one a & Eq a
isPolindrom n
| n == reverse n = True
= False

//Start = isPolindrom 67

prodPali :: [[a]] -> a | * a & one a & Eq a
prodPali [] = one
prodPali [x : ls]
| isPolindrom x = (foldr (*) one x) * prodPali ls 
= prodPali ls 
